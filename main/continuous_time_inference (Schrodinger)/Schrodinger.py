...
class PhysicsInformedNN:
    ...
    def net_f_uv(self, x, t):
        ...
        f_u = u_t - self.D*u_xx + ((self.D*self.eta)/self.L)*u_x
...
if __name__ == "__main__": 
    ...
    Exact_u = np.genfromtxt('u.csv',delimiter=',')
    #Exact_v = np.imag(Exact)
    #Exact_h = np.sqrt(Exact_u**2 + Exact_v**2)
    sol = np.genfromtxt('sol.csv',delimiter=',')
    X, T = np.meshgrid(x,t)
    
    X_star = np.hstack((X.flatten()[:,None], T.flatten()[:,None]))
    u_star = sol.T.flatten()[:,None]
    ...
    u0 = Exact_u.reshape((len(Exact_u), 1))
    #v0 = Exact_v[idx_x,0:1].reshape((len(x), 1))
    
    tb = t.reshape((len(t), 1))
...
#    plt.setp(leg.get_texts(), color='w')
    ...
    ax.plot(x,Exact_u[:,125], 'b-', linewidth = 2, label = 'Exact')       
